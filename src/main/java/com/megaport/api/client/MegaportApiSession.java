...
public class MegaportApiSession {
	...
	public void autoSpeedUpdate(String productUid, Integer measurementHours, Integer margin, Integer floor, Integer ceiling) throws Exception {
		...
		if (serviceObject instanceof VxcServiceDto) {
			...
			if (vxcServiceDto.getaEnd().getOwnerUid().equals(vxcServiceDto.getbEnd().getOwnerUid()))
				customerOwnsBothEnds = true;
	...
	/**
	 * Lock or unlock a port.
	 *
	 * @param productUid Unique product id of existing service
	 * @param status locked or unlocked.
	 * @throws Exception Report any exceptions.
	 */
	public void lockOrUnlock(String productUid, LockStatus status) throws Exception {
		String url = server + "/v2/product/" + productUid + "/lock";
		HttpResponse<JsonNode> response;
		...
		try {
			if (status == LockStatus.LOCKED) {
				...
				response = Unirest.delete(url).header("X-Auth-Token", token).asJson();
			}
			else {
				throw new BadRequestException(String.format("%s is not a recognized LockStatus", status.name()), 400, null);
			}
		} catch (UnirestException e) {
			throw new ServiceUnavailableException("API Server is not available", 503, null);
		}
